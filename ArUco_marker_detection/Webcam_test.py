...
def track(matrix_coefficients, distortion_coefficients):
    if not cap.isOpened():
        raise RuntimeError('Camera not opened. Check the device index or permissions.')
    ...
    while True:
        ...
        if ids is not None and len(ids) > 0:
            print('DICT: APRILTAG_36h11 ids:', ids.flatten().tolist())
        else:
            print('No match. ids: None')
        if ids is None:  # If there are markers found by detector
            print('Marker undetected')
            if rejected_img_points:
                aruco.drawDetectedMarkers(frame, rejected_img_points, borderColor=(0, 0, 255))
        if ids is not None:  # If there are markers found by detector
            ...
            for i in range(0, len(ids)):  # Iterate in markers
             # Estimate pose of each marker and return the values rvec and tvec---different from camera coefficients
                ...
                )  # markerLength width 0.1m
                print('rvec', rvec)
                print('tvec', tvec)
                (rvec - tvec).any()  # get rid of that nasty numpy value array error
                
                aruco.drawDetectedMarkers(frame.copy(), corners, ids)  # Draw A square around the markers
                cv2.drawFrameAxes(frame, matrix_coefficients, distortion_coefficients, rvec, tvec, 0.05)  # Draw Axis, axis length 0.05m
